...
fn a_real_settings_file_re_encodes_byte_for_byte() {
    ...
    assert_eq!(
        settings_dat::encode(&decoded),
        original,
        "re-encoding a Factorio-written file must reproduce it exactly"
    );
}
...
fn a_real_settings_file_has_the_expected_shape() {
    ...
    for section in ["startup", "runtime-global", "runtime-per-user"] {
        assert!(
            listing.keys().any(|(s, _)| s == section),
...
fn adding_our_trigger_leaves_every_other_setting_untouched() {
    ...
    let after = settings_dat::decode(&settings_dat::encode(&file))
        .expect("re-decodes")
        .listing();
    ...
    assert_eq!(
        after.len(),
        before.len() + 1,
        "exactly one setting should have been added"
    );
    assert_eq!(
        after.get(&("startup".into(), "save-timelapse-headless-scan".into())),
...
fn a_corrupt_file_is_rejected_rather_than_misread() {
    ...
    assert!(
        settings_dat::decode(&damaged).is_err(),
        "a truncated file must fail rather than decode into nonsense"
    );
}
